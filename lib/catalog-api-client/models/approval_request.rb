...
module CatalogApiClient
  class ApprovalRequest
    ...
    attr_accessor :approval_request_ref
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `CatalogApiClient::ApprovalRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `CatalogApiClient::ApprovalRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'approval_request_ref')
        self.approval_request_ref = attributes[:'approval_request_ref']
      end
      ...
      if attributes.key?(:'request_completed_at')
    ...
    def hash
      [id, approval_request_ref, order_item_id, reason, request_completed_at, state].hash
    end
